...
def print_word_length_distributions(collection):
	for i in range(1, 16):
		print "ClassEvent % for {} \
			characters: {}".format(i, word_length_distribution(collection, i))

def word_length_distribution(collection, length):
	list_of_words_in_range = [w for w in collection if len(w) == length]
